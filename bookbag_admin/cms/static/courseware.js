...
function reconvert_confirm(courseware_id) {
    if (confirm('本功能是对上一次转换不成功的PDF文件进行强制转换\n可能导致课件中某些内容不能搜索\n您确定要继续吗？'))
        $.ajax({
            url: '/courseware/convert/' + courseware_id +'/?reconvert&ajax',
            async: false,
            success: function(result) {
                ...
                $('#courseware_action').html('<img width=24px heigh=24px style="padding: 0 0 0 20px"" src="/static/images/loading.gif"></img>');
                $('#reconvert_url').hide();
            }
        });
}
